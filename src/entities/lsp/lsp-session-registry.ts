...
import { registerSessionExecuteCommands } from '@shared/lib/lsp/command-relay'
...
const createSession = async (
     ...
     * `semanticTokens.refreshSupport` capability declared in `buildInitializeParams` (`@shared/lib/lsp/initialize-params.ts`)) — mirrors
 ...
 * command palette's `⌘T` Workspace Symbol search (`use-workspace-symbol-search.ts`), which queries
 * all of a project's active sessions in parallel rather than one language's sessions like
 ...
 * abandoning the wait
 * (`breadcrumbs-bar.tsx`/`outline-panel-container.tsx`/`use-document-symbol-loader.ts`'s
