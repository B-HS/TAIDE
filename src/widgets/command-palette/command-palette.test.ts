 ...
 * either. `command-palette.tsx` transitively touches both (directly, and via
 * `lsp-session-registry.ts`), so both are faked here for the same reasons those other test files
 * fake them, and the module under test is reached through a *dynamic* `import()` so the fakes are
 * registered before its static import graph resolves.
 */
...
const importCommandPalette = () => import('@widgets/command-palette/command-palette')

const PROJECT_ID = 'project-1' as ProjectId
const SERVER_A = 'server-a' as LspServerId
const SERVER_B = 'server-b' as LspServerId

describe('buildDocumentSymbolWaiters (command-palette) — root-aware 소비처 전환 (contract §1.2)', () => {
    test('서버별로 resolveLspRoot 가 반환한 루트를 그대로 waitForSession 에 넘긴다(다중 루트 정확한 선택)', async () => {
        const { buildDocumentSymbolWaiters } = await importCommandPalette()
