...
describe('matchesKeymapEntry', () => {
    test('mac 에서 mod 는 metaKey 로 매칭된다', () => {
        const entry: KeymapEntry = { id: 'quick-open', key: 'p', mods: ['mod'], description: '퀵 오픈' }
    ...
    test('ctrl 수식키는 플랫폼과 무관하게 항상 물리 Ctrl 로만 매칭된다', () => {
        const entry: KeymapEntry = { id: 'git', key: 'g', mods: ['ctrl', 'shift'], description: 'Git' }
    ...
    test('수식키 조합(shift·alt)이 정확히 일치해야 한다', () => {
        const entry: KeymapEntry = { id: 'search', key: 'f', mods: ['mod', 'shift'], description: '검색' }
    ...
    test('키 문자가 다르면 매칭되지 않는다', () => {
        const entry: KeymapEntry = { id: 'save', key: 's', mods: ['mod'], description: '저장' }
