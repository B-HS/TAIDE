...
import { registerSnippetCompletionsForLanguages } from '@shared/lib/snippet-completion'
 ...
 *
 * Newly registered ids are also handed to `registerSnippetCompletionsForLanguages` — snippet
 * completions are registered per exact language id, so without this a plugin language had no
 * snippet provider at all (audit §4-B D6). Done here, next to the monaco registration itself, so
 * both callers of this function get it.
 */
export const registerPluginLanguages = (plugins: LoadedPlugin[]) => {
    const newlyRegisteredLanguageIds: string[] = []
    ...
    for (const plugin of plugins) {
        ...
        for (const language of plugin.manifest.contributes?.languages ?? []) {
            ...
            newlyRegisteredLanguageIds.push(language.id)
        }
    }

    registerSnippetCompletionsForLanguages(newlyRegisteredLanguageIds)
}
