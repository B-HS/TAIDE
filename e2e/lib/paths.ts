 ...
 * still outside the Vite root; `Library/Caches/dev.taide.app.dev/` co-locates the throwaway
 * trees with the app's own cache namespace (this harness is already macOS-only — see
 * {@link REMOTE_LOG_PATH}).
 *
 * The identifier is `dev.taide.app.dev`, not the installed app's `dev.taide.app` (contract d-49):
 * the harness only ever drives a `bun run tauri dev` instance (`e2e-harness.md` §0 — "하네스는
 * 앱을 절대 기동하지 않는다"), and `scripts/tauri.ts` injects the `dev.taide.app.dev` identifier
 * overlay (`src-tauri/tauri.dev.conf.json`) for exactly that subcommand so its `app_data_dir`/log
 * dir never collides with an installed release build's.
 */
export const E2E_FIXTURE_PROJECTS_DIR = path.join(os.homedir(), 'Library/Caches/dev.taide.app.dev/e2e-fixtures')
...
export const REMOTE_LOG_PATH = path.join(process.env.HOME ?? '', 'Library/Logs/dev.taide.app.dev/TAIDE.log')

export const MACOS_APP_SETTINGS_PATH = path.join(process.env.HOME ?? '', 'Library/Application Support/dev.taide.app.dev/settings.json')
