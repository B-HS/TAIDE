 ...
 * spec never expands `src/` (no click, no `tree_toggle` call), so a pass can only mean quick-open
 * found `src/nested-only.ts` through the project-wide `search_list_files` index
 * (`projectFilesQueryOptions`), not through the tree.
 */
