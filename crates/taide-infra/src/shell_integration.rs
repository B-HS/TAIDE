...
use crate::persist;
use crate::shell_quote::posix_quote;
...
/// the OSC 133 command markers — [`crate::terminal_scan::OutputScanner`] scans the
...
mod tests {
    ...
    fn 백슬래시_퍼센트_공백이_든_경로도_osc7_시퀀스를_왕복한다() {
        for path in ["/repo/a\\b", "/repo/100%/src", "/repo/my project", "/repo/%s\\ndir"] {
            ...
            assert_eq!(
                crate::terminal_scan::scan_once(emitted.as_bytes()).latest_cwd(),
                Some(path),
                "{path} 가 그대로 왕복해야 한다"
            );
        }
