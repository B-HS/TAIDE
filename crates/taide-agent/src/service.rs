...
use crate::constants::{
    ACTIVITY_IDLE_QUIET_MS, ACTIVITY_WORKING_HOLD_MS, AGENT_NAME_CLAUDE, AGENT_NAME_CODEX, AGENT_NAME_GEMINI, AGENT_NAME_OPENCODE,
    AGENT_NAME_PI, AGENT_OSC_MARKER, AGENT_OSC_SENTINEL, AGENT_PROTOCOL_VERSION, AGENT_PROTOCOL_VERSION_ENV_NAME,
    CLAUDE_MANAGED_HOOK_EVENTS, CLAUDE_TERMINAL_SEQUENCE_MIN_VERSION, CODEX_HOOK_COMMAND_TIMEOUT_SECONDS, CODEX_MANAGED_HOOK_EVENTS,
    ECHO_SUPPRESS_MS, GEMINI_HOOK_COMMAND_TIMEOUT_MS, GEMINI_MANAGED_HOOK_EVENTS, HOOKS_HTTP_TIMEOUT_SECONDS, HOOKS_URL_MARKER,
    ...
    HOOK_EVENT_STOP, HOOK_EVENT_STOP_FAILURE, HOOK_EVENT_USER_PROMPT_SUBMIT, HOOK_HANDLER_TYPE_COMMAND, KNOWN_AGENT_NAMES,
...
use taide_model::agent::{AgentActivity, BlockedReason, CliInstallStatus, DetectedAgent, ExternalOpenRequest, HookInstallScope};
...
pub use taide_infra::crypto::constant_time_eq;
...
mod tests {
    use super::*;
    use crate::constants::TEXT_OVERLAP_BYTES;
...
Do you want to proceed? ❯ 1. Yes 2. No Esc to cancel · Tab to amend";
    ...
    fn 실물_pty_바이트로_래치가_걸리고_점멸_동안_유지된다() {
        ...
        let base = Instant::now();
        let mut scanner = taide_infra::terminal_scan::OutputScanner::new();
        let mut signals = claude_signals();
...
printf '\\033]777;notify;taide-agent;{\"v\":1,\"agent\":\"claude\",\"event\":\"stop\"}\\007' > /dev/tty 2>/dev/null; fi; exit 0"
    ...
    fn 소유_파일_소스는_마커_헤더와_이벤트_시퀀스_표를_담는다() {
        for agent_name in [AGENT_NAME_OPENCODE, AGENT_NAME_PI] {
            ...
            for binding in agent_hooks(agent_name).unwrap().bindings {
                ...
                let scanned = taide_infra::terminal_scan::scan_once(sequence.as_bytes());
                assert_eq!(
                    scanned.events,
    ...
    mod scenario_timeline {
        use super::*;
        use taide_infra::terminal_scan::OutputScanner;
