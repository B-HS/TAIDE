...
use taide_infra::{language, root_guard};
...
pub fn guess_language_id(path: &str, language_overlays: &[LanguageOverlay]) -> String {
    ...
    MCP_ONLY_LANGUAGE_ID_BY_EXTENSION.iter().find(|(key, _)| *key == extension).map(|(_, id)| (*id).to_string()).unwrap_or(resolved)
}
...
pub fn workspace_folders(projects: &HashMap<ProjectId, Project>) -> Vec<String> {
    let mut roots: Vec<String> = projects.values().filter(|project| !project.root_missing).map(|project| project.root.clone()).collect();
    roots.sort();
    roots
}
...
fn file_label(path: &str) -> String {
    Path::new(path).file_name().and_then(|name| name.to_str()).map(str::to_string).unwrap_or_else(|| path.to_string())
}
...
pub fn open_editors_snapshot(layouts: &HashMap<ProjectId, ProjectLayout>, language_overlays: &[LanguageOverlay]) -> Vec<OpenEditorEntry> {
    ...
    for layout in layouts.values() {
        let active_tab_id = taide_layout::service::find_leaf(&layout.root, &layout.focused_pane).and_then(|leaf| {
            let taide_model::layout::PaneNode::Leaf { active, .. } = leaf else {
                return None;
            };
            active.clone()
        });

        for root in taide_layout::service::all_roots(layout) {
            for node in taide_layout::service::collect_leaves(root) {
                let taide_model::layout::PaneNode::Leaf { tabs, .. } = node else {
                    continue;
                };
                for tab in tabs {
...
mod tests {
    use super::*;
    use taide_model::ide::IdeDiagnosticSeverity;
    use taide_model::project::Project;
