...
use taide_infra::watch_policy::is_ignored_dir;
use taide_model::error::{AppError, AppErrorKind, AppResult};
use taide_model::file::REFUSED_FILE_BYTES;
use taide_model::search::{ReplaceSkipReason, SearchFileMatches, SearchLineMatch, SearchQuery, SEARCH_MATCH_LIMIT};
...
///
/// The ceiling is `taide_model::file::REFUSED_FILE_BYTES`, the same tier boundary the file service
...
///
/// `taide_infra::watch_policy::IGNORED_DIR_NAMES` is pruned unconditionally (via `filter_entry`)
...
/// `taide_infra::watch_policy::IGNORED_DIR_NAMES`.
///
...
/// appear (`taide_infra::watch_policy::IGNORED_DIR_NAMES` — `.git`, `node_modules`, ...) rather than
...
///
/// It is **not** a superset of the tree service's listing, and this comment used to claim it was. That
...
mod tests {
    use super::*;
    use taide_model::search::{ReplaceSkippedFile, SearchReplaceResult, REPLACE_SKIP_REPORT_LIMIT};
    ...
    /// `taide_infra::watch_policy::IGNORED_DIR_NAMES` used to report zero results, because the walk pruned the
