...
use taide_locale::service as locale_service;
use taide_model::error::{AppError, AppResult};
use taide_model::locale::LocalePack;
use taide_model::paths::AppPaths;
use taide_model::settings::{Settings, SettingsPatch, SETTINGS_SCHEMA_VERSION};
use taide_model::sync::{SyncLocaleEntry, SyncPayload, SyncThemeEntry};
use taide_model::theme::Theme;
use taide_settings::service as settings_service;
use taide_theme::service as theme_service;
...
pub fn settings_to_sync_patch(settings: &Settings) -> SettingsPatch {
    strip_non_syncable(&SettingsPatch {
        ...
        search_on_type: Some(settings.search_on_type),
        search_on_type_debounce_ms: Some(settings.search_on_type_debounce_ms),
        git_sections_collapsed: Some(settings.git_sections_collapsed.clone()),
        git_graph_panel_size_px: Some(settings.git_graph_panel_size_px),
...
mod tests {
    ...
    fn apply_locale_entries는_다운로드한_로케일을_저장한다() {
        ...
        let pack = LocalePack {
            version: taide_model::locale::LOCALE_SCHEMA_VERSION,
