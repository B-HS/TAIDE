...
use crate::providers::ollama::OllamaCloudProvider;
use crate::providers::omlx::OmlxProvider;
use crate::providers::AiProviderClient;
...
mod tests {
    ...
    fn 빈_토큰은_저장을_거부한다() {
        ...
        let result = crate::test_runtime::block_on(set_token(&store, &client, AiProviderId::OllamaCloud, "   ".to_string()));
    ...
    fn ollama_토큰은_원문_그대로_저장된다() {
        ...
        crate::test_runtime::block_on(set_token(&store, &client, AiProviderId::OllamaCloud, "raw-key".to_string())).unwrap();
    ...
    fn omlx_토큰은_검증없이_원문_그대로_저장된다() {
        ...
        crate::test_runtime::block_on(set_token(&store, &client, AiProviderId::Omlx, "omlx-key".to_string())).unwrap();
    ...
    fn omlx는_빈_토큰_입력을_해제로_간주한다() {
        ...
        crate::test_runtime::block_on(set_token(&store, &client, AiProviderId::Omlx, "   ".to_string())).unwrap();
    ...
    fn ollama_키가_없으면_모델_조회는_설정_필요_에러를_반환한다() {
        ...
        let result = crate::test_runtime::block_on(list_models(&store, &client, AiProviderId::OllamaCloud, None));
    ...
    fn codex_자격증명이_손상되면_내부_에러를_반환한다() {
        ...
        let result = crate::test_runtime::block_on(list_models(&store, &client, AiProviderId::Codex, None));
    ...
    fn omlx는_base_url이_없으면_모델_조회가_설정_필요_에러를_반환한다() {
        ...
        let result = crate::test_runtime::block_on(list_models(&store, &client, AiProviderId::Omlx, None));
