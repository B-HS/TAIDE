...
use taide_infra::clock::{now_epoch_ms, MS_PER_SECOND};
use taide_infra::language::{self, LanguageOverlay};
...
pub fn read_raw(path: &Path) -> AppResult<Vec<u8>> {
    ...
    if metadata.len() > READ_ONLY_FILE_BYTES {
        return Err(AppError::localized(
            AppErrorKind::InvalidArgument,
...
mod tests {
    ...
    fn read_raw_는_미리보기_상한을_넘는_파일을_거부한다() {
        ...
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(&file, vec![0x41u8; READ_ONLY_FILE_BYTES as usize + 1]).unwrap();
