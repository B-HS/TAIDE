...
fn locate_tree_of_pane(layout: &ProjectLayout, pane_id: &PaneId) -> Option<PaneTreeRef> {
    ...
    layout.auxiliary_windows.iter().position(|window| contains_pane(&window.root, pane_id)).map(PaneTreeRef::Auxiliary)
}
...
fn locate_tree_of_tab(layout: &ProjectLayout, tab_id: &TabId) -> Option<PaneTreeRef> {
    ...
    layout.auxiliary_windows.iter().position(|window| find_tab(&window.root, tab_id).is_some()).map(PaneTreeRef::Auxiliary)
}
...
fn find_tab_mut_in_layout<'a>(layout: &'a mut ProjectLayout, tab_id: &TabId) -> Option<&'a mut Tab> {
    ...
    layout.auxiliary_windows.iter_mut().find_map(|window| find_tab_mut(&mut window.root, tab_id))
}
...
pub fn default_layout() -> ProjectLayout {
    let welcome =
        Tab { id: TabId::new(), kind: TabKind::Welcome, title: "Welcome".to_string(), pinned: false, preview: false, dirty: false, view_state: None };
    let terminal = Tab {
        id: TabId::new(),
        kind: TabKind::Terminal { session_id: String::new(), cwd: None },
    ...
    let root = PaneNode::Leaf { id: leaf_id.clone(), tabs: vec![welcome, terminal], active: Some(active) };
...
pub fn collect_claude_diff_tab_ids(node: &PaneNode) -> Vec<TabId> {
    match node {
        PaneNode::Leaf { tabs, .. } => tabs.iter().filter(|tab| matches!(tab.kind, TabKind::ClaudeDiff { .. })).map(|tab| tab.id.clone()).collect(),
    ...
    match node {
        PaneNode::Leaf { tabs, active, .. } => {
            ...
            if active.as_ref() == Some(&tab.id) {
                *active = if tabs.is_empty() { None } else { Some(tabs[index.min(tabs.len() - 1)].id.clone()) };
            }
            Some(tab)
        }
...
fn wrap_leaf_in_split(existing: PaneNode, new_leaf: PaneNode, dir: SplitDir, edge: DropEdge) -> PaneNode {
    ...
    PaneNode::Split { id: PaneId::new(), dir, children: vec![first, second], sizes: vec![SPLIT_TOTAL_PERCENT / 2.0, SPLIT_TOTAL_PERCENT / 2.0] }
}
...
fn insert_split_at(node: &mut PaneNode, target: &PaneId, dir: SplitDir, edge: DropEdge, pending: &mut Option<PaneNode>) {
    ...
    if let PaneNode::Split { dir: node_dir, children, sizes, .. } = node {
...
pub fn normalize(root: &mut PaneNode) {
    let placeholder = PaneNode::Leaf { id: PaneId::new(), tabs: Vec::new(), active: None };
        ...
        PaneNode::Split { id, dir, children, sizes } => {
            ...
            for (child, size) in children.into_iter().zip(sizes) {
                ...
                match normalized_child {
                    PaneNode::Split { dir: child_dir, children: grandchildren, sizes: grandchild_sizes, .. } if child_dir == dir => {
                        ...
                        for (grandchild, grandchild_size) in grandchildren.into_iter().zip(grandchild_sizes) {
                            let portion = if total > 0.0 { size * (grandchild_size / total) } else { size / count };
            ...
            if new_children.is_empty() {
                return PaneNode::Leaf { id: PaneId::new(), tabs: Vec::new(), active: None };
            }
            if new_children.len() == 1 {
                return new_children.into_iter().next().unwrap_or_else(|| PaneNode::Leaf { id, tabs: Vec::new(), active: None });
            }

            PaneNode::Split { id, dir, children: new_children, sizes: new_sizes }
        }
    }
}
...
fn pane_not_found(pane_id: &PaneId) -> AppError {
    AppError::localized(AppErrorKind::NotFound, "error.layout.paneNotFound", format!("pane not found: {pane_id}")).with_arg("paneId", pane_id)
}
...
pub fn resolve_default_open_pane(layout: &mut ProjectLayout) -> PaneId {
    ...
    log::info!("focused_pane 이 트리에 없어 첫 leaf 로 폴백했습니다 (이전={stale}, 현재={})", layout.focused_pane);
...
pub fn open_tab(layout: &mut ProjectLayout, pane_id: &PaneId, mut tab: Tab, preview: bool) -> AppResult<TabId> {
    ...
    let id = tab.id.clone();
    if preview {
        if let Some(pos) = tabs.iter().position(|existing| existing.preview && !existing.dirty && !existing.pinned) {
            tabs[pos] = tab;
        } else {
            tabs.push(tab);
        }
...
pub fn convert_untitled_to_file(layout: &mut ProjectLayout, tab_id: &TabId, path: String, title: String) -> AppResult<TabId> {
    ...
    if !tabs.iter().any(|tab| &tab.id == tab_id && matches!(tab.kind, TabKind::Untitled { .. })) {
...
pub fn close_tab(layout: &mut ProjectLayout, tab_id: &TabId) -> AppResult<ClosedTab> {
    ...
    let closed = ClosedTab { tab, pane_id, index: index as u32 };
...
pub fn reopen_closed(layout: &mut ProjectLayout) -> Option<TabId> {
    ...
    let index =
        find_leaf(tree_root(layout, target_tree), &target_pane).map_or(index as usize, |leaf| clamp_to_pinned_zone(leaf, tab.pinned, index as usize));
...
fn retarget_file_tabs(layout: &mut ProjectLayout, from: &str, to: &str) -> TabPathChangeOutcome {
    ...
    visit_open_tabs_mut(layout, &mut |tab| {
        ...
        match moved.iter_mut().find(|entry| entry.from == previous) {
            ...
            None => moved.push(TabPathMove { from: previous, to: next, dirty: tab.dirty }),
        }
    });
    ...
    TabPathChangeOutcome { moved, closed_paths: Vec::new(), layout_changed }
}
...
fn close_file_tabs_under(layout: &mut ProjectLayout, path: &str) -> TabPathChangeOutcome {
    ...
    TabPathChangeOutcome { moved: Vec::new(), closed_paths, layout_changed }
}
...
pub fn activate_tab(layout: &mut ProjectLayout, tab_id: &TabId) -> AppResult<()> {
    ...
    let pane_id = find_tab(tree_root(layout, tree), tab_id)
        ...
        .ok_or_else(|| AppError::NotFound(format!("tab not found: {tab_id}")))?;
    let leaf = find_leaf_mut(tree_root_mut(layout, tree), &pane_id).ok_or_else(|| AppError::NotFound(format!("pane not found: {pane_id}")))?;
...
pub fn move_tab(layout: &mut ProjectLayout, tab_id: &TabId, target_pane: &PaneId, index: usize) -> AppResult<()> {
    let target_tree = locate_tree_of_pane(layout, target_pane).ok_or_else(|| AppError::NotFound(format!("pane not found: {target_pane}")))?;
    let source_tree = locate_tree_of_tab(layout, tab_id).ok_or_else(|| AppError::NotFound(format!("tab not found: {tab_id}")))?;

    let tab = extract_tab(tree_root_mut(layout, source_tree), tab_id).ok_or_else(|| AppError::NotFound(format!("tab not found: {tab_id}")))?;
...
fn leaf_with_tab(tab: Tab) -> PaneNode {
    ...
    PaneNode::Leaf { id: PaneId::new(), tabs: vec![tab], active: Some(active) }
}
...
pub fn resize(layout: &mut ProjectLayout, pane_id: &PaneId, sizes: Vec<f32>) -> AppResult<()> {
    ...
    let node = find_split_mut(tree_root_mut(layout, tree), pane_id).ok_or_else(|| AppError::NotFound(format!("pane not found: {pane_id}")))?;
    let PaneNode::Split { children, sizes: existing, .. } = node else {
        return Err(AppError::Internal("expected split pane".to_string()));
    };
    if sizes.len() != children.len() {
        return Err(AppError::InvalidArgument(format!("sizes length {} does not match children length {}", sizes.len(), children.len())));
    }
    *existing = sizes;
    layout.revision += 1;
    Ok(())
...
pub fn move_tab_to_new_window(layout: &mut ProjectLayout, tab_id: &TabId, slot: u32) -> AppResult<()> {
    ...
    layout.auxiliary_windows.push(AuxWindowLayout {
        slot,
        root: PaneNode::Leaf { id: leaf_id.clone(), tabs: Vec::new(), active: None },
...
pub fn return_auxiliary_window_tabs(layout: &mut ProjectLayout, window_slot: u32) -> bool {
    ...
    ensure_focused_pane_valid(layout);
    let target = layout.focused_pane.clone();
    if let Some(PaneNode::Leaf { tabs: leaf_tabs, active, .. }) = find_leaf_mut(&mut layout.root, &target) {
        let had_active = active.is_some();
        for tab in tabs {
            let merged_into = is_dedupable(&tab.kind).then(|| leaf_tabs.iter_mut().find(|existing| existing.kind == tab.kind)).flatten();
...
fn strip_volatile_node(node: &mut PaneNode) {
    match node {
        PaneNode::Leaf { tabs, active, .. } => {
            let removed_index = active.as_ref().and_then(|active_id| tabs.iter().position(|tab| &tab.id == active_id));
            ...
            if active_was_volatile {
                *active = if tabs.is_empty() { None } else { Some(tabs[removed_index.unwrap_or(0).min(tabs.len() - 1)].id.clone()) };
            }
        }
...
pub fn locate_project_with_tab(layouts: &HashMap<ProjectId, ProjectLayout>, tab_id: &TabId) -> AppResult<ProjectId> {
    layouts
        .iter()
        .find_map(|(project_id, layout)| all_roots(layout).any(|root| find_tab(root, tab_id).is_some()).then(|| project_id.clone()))
        .ok_or_else(|| AppError::NotFound(format!("tab not found: {tab_id}")))
}
...
pub fn locate_project_with_pane(layouts: &HashMap<ProjectId, ProjectLayout>, pane_id: &PaneId) -> AppResult<ProjectId> {
    layouts
        .iter()
        .find_map(|(project_id, layout)| all_roots(layout).any(|root| contains_pane(root, pane_id)).then(|| project_id.clone()))
...
pub fn get_layout_mut<'a>(layouts: &'a mut HashMap<ProjectId, ProjectLayout>, project_id: &ProjectId) -> AppResult<&'a mut ProjectLayout> {
    layouts.get_mut(project_id).ok_or_else(|| AppError::NotFound(format!("layout not found: {project_id}")))
}
...
mod tests {
    ...
    fn 검색_에디터_탭(text: &str) -> Tab {
        Tab {
            id: TabId::new(),
            kind: TabKind::SearchEditor { query: 검색_쿼리(text) },
    ...
    fn 클로드_diff_탭(path: &str) -> Tab {
        Tab {
            id: TabId::new(),
            kind: TabKind::ClaudeDiff { request_id: "req-1".to_string(), path: path.to_string() },
            title: path.to_string(),
            pinned: false,
            preview: false,
            dirty: false,
    ...
    fn 리프(tabs: Vec<Tab>) -> PaneNode {
        ...
        PaneNode::Leaf { id: PaneId::new(), tabs, active }
    }

    fn 가로_분할(children: Vec<PaneNode>) -> PaneNode {
        let sizes = vec![SPLIT_TOTAL_PERCENT / children.len() as f32; children.len()];
        PaneNode::Split { id: PaneId::new(), dir: SplitDir::Horizontal, children, sizes }
    }
    ...
    fn 빈_리프는_정규화에서_제거된다() {
        ...
        let mut root = PaneNode::Split { id: PaneId::new(), dir: SplitDir::Horizontal, children: vec![empty_leaf, full_leaf], sizes: vec![0.5, 0.5] };
    ...
    fn 같은_방향_스플릿에서는_형제로_삽입된다() {
        ...
        let PaneNode::Leaf { id: leaf_id, .. } = &layout.root else { panic!("expected leaf") };
        ...
        let PaneNode::Split { dir, children, .. } = &layout.root else { panic!("expected split") };
        ...
        let PaneNode::Split { dir, children, sizes, .. } = &layout.root else { panic!("expected split") };
        assert_eq!(*dir, SplitDir::Horizontal);
    ...
    fn 루트_리프_id(layout: &ProjectLayout) -> PaneId {
        let PaneNode::Leaf { id, .. } = &layout.root else { panic!("expected leaf") };
        id.clone()
    }
    ...
    fn 새_탭_분할은_루트_리프를_감싸고_새_페인을_포커스한다() {
        ...
        let PaneNode::Leaf { tabs, .. } = &layout.root else { panic!("expected leaf") };
        let 원래_탭_수 = tabs.len();
        ...
        let PaneNode::Leaf { id, tabs, .. } = &children[0] else { panic!("expected existing leaf") };
        assert_eq!(id, &leaf_id);
        assert_eq!(tabs.len(), 원래_탭_수, "기존 페인의 탭은 옮겨지지 않는다");

        let PaneNode::Leaf { id: new_leaf_id, tabs: new_tabs, active } = &children[1] else { panic!("expected new leaf") };
    ...
    fn 새_탭_분할의_left_top_엣지는_새_페인을_기존_페인_앞에_둔다() {
        for (edge, expected_dir) in [(DropEdge::Left, SplitDir::Horizontal), (DropEdge::Top, SplitDir::Vertical)] {
            ...
            let PaneNode::Leaf { tabs, .. } = &children[0] else { panic!("expected new leaf first") };
    ...
    fn 새_탭_분할은_중첩_대상에서도_새_페인을_포커스하고_revision을_한_번만_올린다() {
        ...
        let PaneNode::Split { children, .. } = &layout.root else { panic!("expected split") };
        let PaneNode::Split { children: inner, .. } = &children[0] else { panic!("expected wrapped split") };
        let PaneNode::Leaf { id, tabs, active, .. } = &inner[1] else { panic!("expected new leaf") };
    ...
    fn 새_탭_분할은_기존_페인의_활성_탭을_바꾸지_않는다() {
        let mut layout = default_layout();
        let leaf_id = 루트_리프_id(&layout);
        let PaneNode::Leaf { active, .. } = &layout.root else { panic!("expected leaf") };
        let active_before = active.clone();

        open_tab_in_split(&mut layout, &leaf_id, DropEdge::Right, 파일_탭("a.rs")).expect("open in split");

        let PaneNode::Split { children, .. } = &layout.root else { panic!("expected split") };
        let PaneNode::Leaf { active, .. } = &children[0] else { panic!("expected existing leaf") };
    ...
    fn 새_탭_분할은_다른_방향_부모에서는_대상_리프만_감싼다() {
        ...
        let PaneNode::Split { dir: inner_dir, children: inner, .. } = &children[0] else { panic!("expected wrapped split") };
    ...
    fn 새_탭_분할은_같은_kind_터미널_탭을_중복_제거하지_않는다() {
        ...
        let PaneNode::Split { children, .. } = &layout.root else { panic!("expected split") };
        let PaneNode::Leaf { tabs, .. } = &children[1] else { panic!("expected new leaf") };
    ...
    fn 기존_split_은_추출_이후에도_탭을_원래_페인에서_옮긴다() {
        ...
        let PaneNode::Split { children, .. } = &layout.root else { panic!("expected split") };
        let PaneNode::Leaf { tabs: 남은_탭, .. } = &children[0] else { panic!("expected source leaf") };
        assert_eq!(남은_탭.len(), 원래_탭_수 - 1);
        assert!(남은_탭.iter().all(|tab| tab.id != tab_id));

        let PaneNode::Leaf { tabs: 옮겨진_탭, .. } = &children[1] else { panic!("expected new leaf") };
    ...
    fn 중첩된_같은_방향_스플릿은_평탄화된다() {
        ...
        let inner = PaneNode::Split { id: PaneId::new(), dir: SplitDir::Horizontal, children: vec![b, c], sizes: vec![0.5, 0.5] };
        let mut root = PaneNode::Split { id: PaneId::new(), dir: SplitDir::Horizontal, children: vec![a, inner], sizes: vec![0.5, 0.5] };

        normalize(&mut root);

        let PaneNode::Split { children, sizes, .. } = &root else { panic!("expected split") };
        assert_eq!(children.len(), 3);
        assert_eq!(sizes.len(), 3);
    }
    ...
    fn 세션_터미널_탭(session_id: &str) -> Tab {
        Tab { kind: TabKind::Terminal { session_id: session_id.to_string(), cwd: None }, ..터미널_탭() }
    }

    fn 핀_파일_탭(path: &str) -> Tab {
        Tab { pinned: true, ..파일_탭(path) }
    }
    ...
    fn 탭_제목들(layout: &ProjectLayout, pane_id: &PaneId) -> Vec<String> {
        let Some(PaneNode::Leaf { tabs, .. }) = find_leaf(&layout.root, pane_id) else { panic!("리프가 있어야 한다") };
    ...
    fn 같은_페인에서_핀_탭을_맨_뒤로_끌어도_핀_구역_안에_머문다() {
        ...
        assert_eq!(탭_제목들(&layout, &pane_id), vec!["b.rs".to_string(), "a.rs".to_string(), "c.rs".to_string()]);
    }
    ...
    fn 다른_페인으로_옮긴_핀_탭은_대상_핀_구역_끝에_들어간다() {
        ...
        assert_eq!(탭_제목들(&layout, &target_pane), vec!["pinned.rs".to_string(), "moving.rs".to_string(), "plain.rs".to_string()]);
    }
    ...
    fn 닫은_탭_다시_열기는_핀_구역_뒤로_클램프된다() {
        ...
        assert_eq!(탭_제목들(&layout, &pane_id), vec!["c.rs".to_string(), "d.rs".to_string(), "b.rs".to_string(), "a.rs".to_string()]);
    }
    ...
    fn 닫은_탭_스택은_dirty를_정규화한다() {
        ...
        assert!(!find_tab_mut_in_layout(&mut layout, &a_id).expect("되살린 탭").dirty, "되살린 탭에 유령 dirty 가 남으면 안 된다");
    }

    fn 보조_창_리프(layout: &ProjectLayout, slot: u32) -> PaneId {
        layout.auxiliary_windows.iter().find(|window| window.slot == slot).expect("보조 창").focused_pane.clone()
    }
    ...
    fn 보조_창_회수는_같은_파일_탭을_합치고_dirty를_승계한다() {
        ...
        let aux_a = Tab { dirty: true, ..파일_탭("a.rs") };
    ...
    fn 보조_창에서_명시적으로_연_탭이_합쳐지면_main_프리뷰_탭을_승격시킨다() {
        ...
        let Some(PaneNode::Leaf { tabs, .. }) = find_leaf(&layout.root, &pane_id) else { panic!("리프가 있어야 한다") };
        let survivor = tabs.iter().find(|tab| tab.id == main_a_id).expect("main 탭이 살아남아야 한다");
        assert!(!survivor.preview, "명시적으로 연 탭이 프리뷰 탭과 합쳐지면 생존 탭은 프리뷰에서 승격돼야 한다");
    }
    ...
    fn 미러가_없는_보조_창_파일_탭의_dirty는_복귀_전에_해제된다() {
        ...
        move_tab_to_new_window(&mut layout, &anchor_id, slot).expect("move anchor to new window");
        let aux_pane = 보조_창_리프(&layout, slot);
        let mirrored = Tab { dirty: true, ..파일_탭("mirrored.rs") };
        let mirrored_id = mirrored.id.clone();
        let phantom = Tab { dirty: true, ..파일_탭("phantom.rs") };
        ...
        let Some(PaneNode::Leaf { tabs, .. }) = find_leaf(&window.root, &aux_pane) else { panic!("보조 창 리프가 있어야 한다") };
    ...
    fn 유령_dirty_해제는_파일이_아닌_탭과_다른_창을_건드리지_않는다() {
        let main_dirty = Tab { dirty: true, ..파일_탭("main-only.rs") };
        ...
        move_tab_to_new_window(&mut layout, &anchor_id, slot).expect("move anchor to new window");
        let aux_pane = 보조_창_리프(&layout, slot);
        let aux_terminal = Tab { dirty: true, ..터미널_탭() };
    ...
    fn 보조_창에서_돌아온_핀_탭은_핀_구역으로_들어간다() {
        ...
        assert_eq!(탭_제목들(&layout, &pane_id), vec!["c.rs".to_string(), "a.rs".to_string(), "b.rs".to_string(), "d.rs".to_string()]);
    }

    #[test]
    fn 같은_파일_재열기는_활성화만_한다() {
        let mut layout = default_layout();
        let PaneNode::Leaf { id: leaf_id, tabs, .. } = &layout.root else { panic!("expected leaf") };
        let leaf_id = leaf_id.clone();
        ...
        let PaneNode::Leaf { tabs, active, .. } = &layout.root else { panic!("expected leaf") };
    ...
    fn 스플릿_후_sizes_길이는_children_길이와_일치한다() {
        ...
        let PaneNode::Split { children, sizes, .. } = &layout.root else { panic!("expected split") };
    ...
    fn 리사이즈는_sizes_길이가_다르면_실패한다() {
        ...
        let PaneNode::Split { id: split_id, .. } = &layout.root else { panic!("expected split") };
    ...
    fn 저장시_클로드_diff_탭은_필터링되고_활성탭은_인접탭으로_넘어간다() {
        ...
        let root = PaneNode::Leaf { id: PaneId::new(), tabs: vec![file_tab, claude_diff_tab], active: Some(claude_diff_tab_id) };
        ...
        let PaneNode::Leaf { tabs, active, .. } = &persisted.root else { panic!("expected leaf") };
        assert_eq!(tabs.len(), 1);
    ...
    fn 클로드_diff_탭이_없으면_저장_대상이_그대로_유지된다() {
        ...
        let PaneNode::Leaf { tabs, .. } = &persisted.root else { panic!("expected leaf") };
        assert_eq!(tabs.len(), 2);
    }
    ...
    fn 저장시_닫힌_탭_스택에서도_클로드_diff_탭이_제거된다() {
        let file_closed = ClosedTab { tab: 파일_탭("a.rs"), pane_id: PaneId::new(), index: 0 };
        let claude_diff_closed = ClosedTab { tab: 클로드_diff_탭("b.rs"), pane_id: PaneId::new(), index: 1 };
        let layout = ProjectLayout {
            version: LAYOUT_SCHEMA_VERSION,
    ...
    fn 클로드_diff_탭만_있던_리프는_저장시_정규화로_제거된다() {
        ...
        let root =
            PaneNode::Split { id: PaneId::new(), dir: SplitDir::Horizontal, children: vec![claude_only_leaf, file_leaf], sizes: vec![0.5, 0.5] };
        let layout = ProjectLayout {
            version: LAYOUT_SCHEMA_VERSION,
            root,
            focused_pane: PaneId::new(),
    ...
    fn 포커스되지_않은_페인의_마지막_탭을_닫아도_포커스는_그대로다() {
        ...
        assert!(find_leaf(&layout.root, &pane_ids[2]).is_none());
        assert_eq!(layout.focused_pane, pane_ids[0], "포커스 아닌 페인이 사라져도 포커스는 그 페인의 형제로 옮겨가지 않는다");
    }
    ...
    fn 파일_삭제로_포커스_페인이_비어도_focused_pane은_유효하다() {
        ...
        let outcome = apply_tab_path_change(&mut layout, &TabPathChange::Deleted { path: "/repo/src".to_string() });
    ...
    fn 저장하면_같은_자리에서_file_탭이_된다() {
        ...
        let converted_id = convert_untitled_to_file(&mut layout, &untitled_id, "notes.md".to_string(), "notes.md".to_string()).expect("convert");
        ...
        assert_eq!(tab.kind, TabKind::File { path: "notes.md".to_string() });
    ...
    fn 같은_쿼리의_검색_에디터_재열기는_활성화만_한다() {
        ...
        let PaneNode::Leaf { tabs, active, .. } = &layout.root else { panic!("expected leaf") };
        assert_eq!(first_id, second_id);
        assert_eq!(tabs.iter().filter(|tab| matches!(tab.kind, TabKind::SearchEditor { .. })).count(), 1);
        assert_eq!(active, &Some(first_id));
    }
    ...
    fn 다른_쿼리의_검색_에디터는_별도_탭으로_열린다() {
        ...
        let PaneNode::Leaf { tabs, .. } = &layout.root else { panic!("expected leaf") };
        assert_eq!(tabs.iter().filter(|tab| matches!(tab.kind, TabKind::SearchEditor { .. })).count(), 2);
    }
    ...
    fn v1_레이아웃_파일은_기존_탭을_보존한채_v2로_마이그레이션된다() {
        ...
        std::fs::write(paths.layout_file(&project_id), serde_json::to_vec_pretty(&raw).expect("serialize v1 json")).expect("write v1 layout file");
        ...
        let PaneNode::Leaf { tabs, .. } = &loaded.root else { panic!("expected leaf") };
        assert_eq!(tabs.len(), 3, "welcome + terminal + kept.rs 가 모두 보존되어야 한다");
        assert!(tabs.iter().any(|tab| matches!(&tab.kind, TabKind::File { path } if path == "kept.rs")));
    ...
    fn appfile_탭의_dirty는_불러올때_초기화된다() {
        ...
        app_file_tab.kind = TabKind::AppFile { target: AppFileTarget::Settings };
        ...
        let PaneNode::Leaf { tabs, .. } = &loaded.root else { panic!("expected leaf") };
        let app_file_tab = tabs.iter().find(|tab| matches!(tab.kind, TabKind::AppFile { .. })).expect("appFile 탭이 보존되어야 한다");
        assert!(!app_file_tab.dirty, "미러가 없는 AppFile 탭의 dirty 는 재시작 후 유령으로 남으면 안 된다");
    ...
    fn 새_보조_창으로_탭을_이동하면_dirty_pinned_view_state가_보존된다() {
        ...
        let PaneNode::Leaf { tabs, .. } = &window.root else { panic!("expected leaf") };
        assert_eq!(tabs.len(), 1);
        assert!(tabs[0].dirty);
        assert!(tabs[0].pinned);
    ...
    fn 기존_보조_창으로_탭을_이동할_수_있다() {
        ...
        let window = layout.auxiliary_windows.iter().find(|window| window.slot == slot).expect("window exists");
        let PaneNode::Leaf { tabs, .. } = &window.root else { panic!("expected leaf") };
        assert_eq!(tabs.len(), 2);
    }
    ...
    fn 보조_창의_마지막_탭을_main으로_되돌리면_창이_비워진다() {
        ...
        let window = layout.auxiliary_windows.iter().find(|window| window.slot == slot).expect("window still recorded");
    ...
    fn 보조_창_내부에서도_스플릿과_탭_열기가_그_창의_트리에만_반영된다() {
        ...
        let window = &layout.auxiliary_windows[0];
        assert!(matches!(&window.root, PaneNode::Split { .. }), "보조 창 내부 스플릿이 그 창의 트리에 반영되어야 한다");
    ...
    fn shell_view_패치는_none_필드를_보존한다() {
        let mut layout = default_layout();
        apply_shell_view_patch(&mut layout, &ShellViewPatch { zen: Some(true), sidebar_collapsed: None });
        assert!(layout.shell_view.zen);
        assert!(!layout.shell_view.sidebar_collapsed);

        apply_shell_view_patch(&mut layout, &ShellViewPatch { zen: None, sidebar_collapsed: Some(true) });
    ...
    fn locate_project_with_tab_과_pane_은_보조_창_트리도_찾는다() {
        ...
        assert_eq!(locate_project_with_pane(&layouts, &source_pane).expect("main 트리의 pane 은 여전히 찾아야 한다"), project_id);

        assert!(locate_project_with_tab(&layouts, &TabId::new()).is_err(), "존재하지 않는 탭은 NotFound 여야 한다");
    }
    ...
    fn open_file_paths는_메인과_보조창의_파일_탭_경로를_중복없이_수집한다() {
        ...
        assert_eq!(paths, vec!["a.rs".to_string(), "b.rs".to_string()], "메인·보조 창에 걸쳐 열린 파일 탭 경로가 중복 없이 모여야 한다");
    }

    #[test]
    fn open_file_paths는_파일_탭이_없으면_비어있다() {
        let layout = default_layout();
        assert!(open_file_paths(&layout).is_empty(), "기본 레이아웃(웰컴·터미널 탭만)에는 파일 탭이 없어야 한다");
    }

    fn 개명(from: &str, to: &str) -> TabPathChange {
        TabPathChange::Renamed { from: from.to_string(), to: to.to_string() }
    }
    ...
    fn 파일_개명은_열린_탭의_경로와_제목을_따라간다() {
        ...
        assert_eq!(tab.kind, TabKind::File { path: "/repo/src/new.rs".to_string() });
        assert_eq!(tab.title, "new.rs", "탭 제목도 새 파일 이름을 따라야 한다");
        assert_eq!(outcome.moved, vec![TabPathMove { from: "/repo/src/old.rs".to_string(), to: "/repo/src/new.rs".to_string(), dirty: false }]);
    ...
    fn 폴더_개명은_하위_열린_탭_전체의_경로를_치환한다() {
        ...
        assert_eq!(
            paths,
            vec!["/repo/lib/a.rs".to_string(), "/repo/lib/nested/b.rs".to_string(), "/repo/other/c.rs".to_string(),],
    ...
    fn 같은_파일이_두_페인에_열려_있으면_한_번만_보고하고_dirty_를_합친다() {
        ...
        assert_eq!(
            outcome.moved,
            vec![TabPathMove { from: "/repo/a.rs".to_string(), to: "/repo/b.rs".to_string(), dirty: true }],
    ...
    fn 개명은_닫은_탭_스택의_경로도_따라간다() {
        ...
        assert_eq!(closed.tab.kind, TabKind::File { path: "/repo/lib/old.rs".to_string() });
    ...
    fn 대상_탭이_없으면_revision_을_올리지_않는다() {
        ...
        let deleted = apply_tab_path_change(&mut layout, &TabPathChange::Deleted { path: "/repo/gone.rs".to_string() });

        assert!(renamed.is_empty() && deleted.is_empty());
        assert!(!renamed.layout_changed && !deleted.layout_changed, "매칭된 것이 하나도 없으면 레이아웃 자체가 바뀌지 않았다");
    ...
    fn 열린_탭이_없어도_닫은_탭_스택의_경로는_따라가고_레이아웃_변경으로_보고된다() {
        ...
        assert!(outcome.layout_changed, "닫은 탭 스택이 새 경로로 바뀌었으므로 이 레이아웃은 저장돼야 한다 (커맨드의 조기 반환이 버리면 안 된다)");
        assert_eq!(layout.closed_tabs.last().expect("closed tab exists").tab.kind, TabKind::File { path: "/repo/b.txt".to_string() });
