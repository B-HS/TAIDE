...
impl ScrollbackRing {
    ...
    fn line_aligned_eviction_end(&self, overflow: usize) -> usize {
        ...
        self.buffer.iter().take(scan_end).skip(scan_start).position(|byte| *byte == b'\n').map_or(overflow, |offset| scan_start + offset + 1)
    }
...
const HOMEBREW_CANDIDATE_SHELLS: &[&str] =
    &["/opt/homebrew/bin/fish", "/opt/homebrew/bin/zsh", "/opt/homebrew/bin/nu", "/usr/local/bin/fish", "/usr/local/bin/zsh", "/usr/local/bin/nu"];
    ...
    if let Ok(content) = std::fs::read_to_string("/etc/shells") {
        for path in parse_shell_list(&content) {
            if seen.insert(path.clone()) {
                profiles.push(ShellProfile { id: path.clone(), name: shell_name(&path), path, args: Vec::new() });
            }
        }
    }
...
pub fn list_shell_profiles() -> Vec<ShellProfile> {
    ...
    vec![ShellProfile { id: "cmd".to_string(), name: "Command Prompt".to_string(), path: comspec, args: Vec::new() }]
}
...
pub fn resolve_terminal_path(raw: &str, cwd: &str) -> AppResult<String> {
    ...
    let resolved = if candidate.is_absolute() { candidate } else { PathBuf::from(cwd).join(candidate) };
...
mod tests {
    ...
    fn 상한_밖의_개행까지는_축출을_전진시키지_않는다() {
        ...
        assert_eq!(retained.len(), CAPACITY, "상한을 넘는 개행 탐색은 포기하고 원래 위치에서 잘라야 한다");
    ...
    fn 경로_라인_컬럼을_전부_해석한다() {
        assert_eq!(parse_terminal_path("src/main.rs:42:10"), ("src/main.rs".to_string(), Some(42), Some(10)));
    }
    ...
    fn 윈도우_드라이브_콜론은_경로에_보존된다() {
        assert_eq!(parse_terminal_path("C:\\foo\\bar.rs:5"), ("C:\\foo\\bar.rs".to_string(), Some(5), None));
    }
