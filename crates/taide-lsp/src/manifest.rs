...
use taide_model::lsp::{LanguageServerSpec, LspInstallStrategy, LspManifest};
...
pub fn parse_manifest(source: &str) -> Result<Vec<LanguageServerSpec>, String> {
    let manifest: LspManifest = serde_json::from_str(source)
        .map_err(|error| format!("lsp manifest parse 실패: {error}"))?;
...
fn resolve_servers(source: &str) -> Vec<LanguageServerSpec> {
    match parse_manifest(source) {
        ...
        Err(error) => {
            log::error!(
                "번들된 lsp manifest 파싱에 실패했습니다. 빈 서버 목록으로 대체합니다: {error}"
            );
            Vec::new()
        }
    }
...
pub fn servers() -> Vec<LanguageServerSpec> {
    BUNDLED_SERVERS
        .get_or_init(|| resolve_servers(LSP_MANIFEST_SOURCE))
        .clone()
}
...
fn validate_manifest(specs: &[LanguageServerSpec]) -> Result<(), String> {
    ...
    for spec in specs {
        ...
        if !seen_ids.insert(spec.id.clone()) {
            return Err(format!(
                "lsp manifest: server id가 중복되었습니다: {}",
                spec.id
            ));
        }
        if spec.language_ids.is_empty() {
            return Err(format!(
                "lsp manifest: {} 의 languageIds가 비어있습니다",
                spec.id
            ));
        }
...
fn validate_install(spec: &LanguageServerSpec) -> Result<(), String> {
        ...
        LspInstallStrategy::Toolchain => {
            if let Some(toolchain) = &spec.install.toolchain {
                if toolchain.tool_detect.command.is_empty() {
                    return Err(format!(
                        "lsp manifest: {} 의 toolchain.toolDetect.command 가 비어있습니다",
                        spec.id
                    ));
                }
            }
    ...
    match &spec.install.sdk_detect {
        Some(sdk_detect) if sdk_detect.probes.is_empty() => {
            return Err(format!(
                "lsp manifest: {} 의 sdkDetect.probes 가 비어있습니다",
                spec.id
            ));
        }
