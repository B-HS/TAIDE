...
use taide_model::lsp::{
    LanguageServerSpec, LspRootStrategy, LspSdkProbe, LspServerDetection, LspToolchainTool,
};
...
fn requires_server_dir(spec: &LanguageServerSpec) -> bool {
    spec.command
        .args()
        .iter()
        .any(|arg| arg.contains(SERVER_DIR_TEMPLATE))
}
...
fn toolchain_extra_search_dirs(tool: LspToolchainTool) -> Vec<PathBuf> {
    ...
    match tool {
        ...
        LspToolchainTool::Coursier => home
            ...
            .unwrap_or_default(),
        LspToolchainTool::Ghcup => home
            .map(|home| vec![home.join(".ghcup/bin")])
            .unwrap_or_default(),
    }
}
...
pub fn resolve_spec_command(
    ...
    if let Some(resolved) = resolve_command(
        spec.command.bin(),
        root,
        managed_dir,
    ...
    toolchain_extra_search_dirs(toolchain.tool)
        .into_iter()
        .find_map(|dir| {
...
pub fn toolchain_binary(tool: LspToolchainTool) -> &'static str {
    match tool {
...
fn evaluate_sdk_probes(probes: &[LspSdkProbe], path_var: &OsStr) -> bool {
    probes.iter().any(|probe| match probe {
        ...
        LspSdkProbe::Xcrun { tool } => {
            find_in_path_within("xcrun", path_var).is_some()
                && find_in_path_within(tool, path_var).is_some()
        }
    })
}
...
pub fn detect_servers(lsp_dir: &Path, path_var: &OsStr) -> Vec<LspServerDetection> {
    manifest::servers()
        ...
        .map(|spec| {
            ...
            let resolved = resolve_spec_command(
                &spec,
                None,
            ...
            let toolchain_available = spec.install.toolchain.as_ref().map(|toolchain| {
                find_in_path_within(toolchain_binary(toolchain.tool), path_var).is_some()
            });
            ...
            let download_available = spec.install.download.as_ref().map(|download| {
                ...
                download.urls.contains_key(&platform)
                    && download.sha256.get(&platform).cloned().flatten().is_some()
            });
...
fn lowercase_drive_letter(path: String) -> String {
    ...
    let has_uppercase_drive = bytes.len() >= DRIVE_PREFIX_LEN
        && bytes[0] == b'/'
        && bytes[2] == b':'
...
pub fn workspace_folder_uri(root: &str) -> String {
    ...
    format!(
        "{FILE_URI_SCHEME}{}",
        encode_uri_path(&lowercase_drive_letter(rooted))
    )
}
...
mod tests {
    use super::*;
    use taide_model::lsp::LspInstallStrategy;
    ...
    fn 외부_러스트_소스_경로를_식별한다() {
        ...
        assert!(is_external_rust_source(Path::new(
            "/Users/me/.rustup/toolchains/stable/lib/std.rs"
        )));
        assert!(!is_external_rust_source(Path::new(
    ...
    fn detect_servers는_매니페스트의_initialization_options을_그대로_전달한다() {
        ...
        let rust_analyzer = detections
            .iter()
            .find(|detection| detection.id == "rustAnalyzer")
            .unwrap();
        assert_eq!(
            rust_analyzer
        ...
        let vtsls = detections
            .iter()
            .find(|detection| detection.id == "vtsls")
            .unwrap();
    ...
    fn detect_servers는_매니페스트의_모든_서버_결과를_반환한다() {
        ...
        assert!(original_five
            .iter()
            .all(|id| detections.iter().any(|detection| detection.id == *id
                && detection.install_strategy == LspInstallStrategy::SdkDetect)));
        std::fs::remove_dir_all(&lsp_dir).ok();
    }
    ...
    fn detect_servers는_toolchain과_sdk_detect_전략도_보고한다() {
        ...
        let gopls = detections
            .iter()
            .find(|detection| detection.id == "gopls")
            .unwrap();
        ...
        let sourcekit = detections
            .iter()
            .find(|detection| detection.id == "sourcekitLsp")
            .unwrap();
    ...
    fn detect_servers는_download_전략에_딸린_sdk_전제조건도_평가한다() {
        ...
        let jdtls = detections
            .iter()
            .find(|detection| detection.id == "jdtls")
            .unwrap();
    ...
    fn jdtls는_관리_디렉토리가_없으면_path_에_java_가_있어도_available_이_아니다() {
        ...
        assert!(
            !jdtls.available,
            "managed 디렉토리 없이는 java 런처만으로 available 이 되면 안된다"
        );
    ...
    fn detect_servers는_관리_디렉토리에_설치된_버전을_보고한다() {
        ...
        let detections = detect_servers(&lsp_dir, &real_path_var());
        let marksman = detections
            .iter()
            .find(|detection| detection.id == "marksman")
            .unwrap();
    ...
    fn decode_file_uri(uri: &str) -> String {
        ...
        let path = uri
            ...
            .expect("file 스킴이어야 한다");
        let bytes = path.as_bytes();
        let mut decoded = Vec::with_capacity(bytes.len());
        let mut index = 0;
        while index < bytes.len() {
            if bytes[index] == b'%' {
                let hex = std::str::from_utf8(&bytes[index + 1..index + PERCENT_ESCAPE_LEN])
                    .expect("이스케이프는 ascii 여야 한다");
    ...
    fn 공백과_한글이_있는_루트는_프론트와_같은_퍼센트_인코딩_uri가_된다() {
        assert_eq!(
            workspace_folder_uri("/workspace/my project"),
    ...
    fn 인코딩한_루트는_원래_경로로_되돌아온다() {
        for root in [
            ...
            assert_eq!(
                decode_file_uri(&workspace_folder_uri(root)),
                root,
                "{root} 왕복 실패"
            );
        }
    ...
    fn 예약되지_않은_문자와_경로_구분자는_그대로_둔다() {
        assert_eq!(
            workspace_folder_uri("/tmp/tilde~dash-dot._x"),
            "file:///tmp/tilde~dash-dot._x"
        );
    }
    ...
    fn 하위_구분자와_예약_문자도_프론트처럼_인코딩한다() {
        assert_eq!(
            workspace_folder_uri("/tmp/a+b&c,d;e=f"),
            "file:///tmp/a%2Bb%26c%2Cd%3Be%3Df"
        );
        assert_eq!(
        ...
        assert_eq!(
            workspace_folder_uri("/tmp/100% sure"),
            "file:///tmp/100%25%20sure"
        );
    }
    ...
    fn 드라이브_문자는_소문자로_맞추고_상대_경로는_루트를_붙인다() {
        assert_eq!(
            workspace_folder_uri("/C:/Users/a b"),
            "file:///c%3A/Users/a%20b"
        );
        assert_eq!(
            workspace_folder_uri("workspace/project"),
