...
use super::types::{
    ALLOWED_HOST_WILDCARD_PREFIX, REMOTE_LINK_TOKEN_QUERY_KEY, REMOTE_LOOPBACK_HOSTNAMES,
    REMOTE_PASSWORD_MIN_LEN,
};
...
pub fn is_allowed_host(host: Option<&str>, allowed_hosts: &[String], bind_port: u32) -> bool {
    ...
    allowed_hosts
        .iter()
        .any(|allowed| host_matches_allowed_entry(&hostname, allowed))
}
...
mod tests {
    ...
    fn 와일드카드_등록은_선두_한_레이블만_허용한다() {
        ...
        assert!(is_allowed_host(
            Some("foo.trycloudflare.com:53211"),
            &allowed,
            53_211
        ));
    }
    ...
    fn 와일드카드_등록은_레이블이_두_개_이상이면_거부한다() {
        let allowed = vec!["*.trycloudflare.com".to_string()];
        assert!(!is_allowed_host(
            Some("a.b.trycloudflare.com:53211"),
            &allowed,
            53_211
        ));
    }
    ...
    fn 등록된_호스트가_없으면_루프백_http_링크를_발급한다() {
        assert_eq!(
            format_issue_link_url(&[], 53_211, "tok123"),
