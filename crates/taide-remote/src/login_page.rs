use taide_locale::service::{builtin_by_id, builtin_en};
...
pub fn render(params: LoginPageParams) -> String {
    ...
    let status_html = if let Some(remaining_seconds) = params.locked_remaining_seconds {
        let locked_text = escape_html(
            &message(&pack, "remote.loginLocked")
                .replace("{{seconds}}", &remaining_seconds.to_string()),
        );
