...
- 질의는 `fuzzyFilter` 가 공백으로 최대 8토큰까지 쪼개, 2토큰 이상이면 **모든 토큰이 각각 매칭될
  때만** 통과시킨다(순서 무관 · 점수는 합 · 강조 인덱스는 합집합). 단일 토큰은 기존 동작
  그대로고, 빈 질의·공백뿐인 질의는 전체를 원 순서로 돌려준다 — 공백 없는 파일 경로가
  `search panel` 같은 질의에 0건이던 문제를 막는다(d-55 §1.C).
