...
- query: `search_list_files(projectId) → string[]`(d-42, 2026-08-25 신규) — 프로젝트 root 이하 전체
  ...
  트리가 보여주는 파일과 퀵오픈이 찾는 파일의 집합을 항상 일치시키기 위함이었다(`search_run`/
...
- **채널 다중화(내부 구현, 새 IPC 표면 아님) — pty 는 다중 창 구독, LSP 는 창별 독립 세션(정정,
  ...
  `subscribers.contains(owner)`(호출자 자신이 이미 그 세션의 유일한 구독자일 것)를 포함하므로,
...
- **`fs:changed` 의 무시 규칙(계약 정정 — §4-A-4)**: 무시 목록(`IGNORED_DIR_NAMES`)은 경로의
  **조상 디렉토리 성분에만** 적용된다. 마지막 성분(이벤트의 대상 항목 자신)은 판정에서 제외되므로,
  루트의 `build`·`dist` 같은 **이름을 가진 파일**의 변경도 이제 이벤트로 도착한다(그 파일은 트리에
  원래 보였고 — d-64 T1 이후로는 트리가 무시 목록을 아예 적용하지 않는다 — 변경만 영원히 반영되지
  않았다). 부수 효과로
...
- **프론트 무효화 집합**(`app/providers/ipc-sync-provider.tsx` — `rescanInvalidations`): 경로 목록이
  ...
  - **`FILE.ALL` 만 프로젝트 스코프가 아니다**(검토 F2): `FILE.CONTENT`/`FILE.RAW` 는 `ProjectId`
    축 없이 경로만으로 키가 잡히므로, 이 한 항목의 무효화는 **다른 프로젝트에서 열어 둔 파일까지**
    디스크에서 다시 읽게 한다. 좁히려면 `projectClosed` 처럼 `PROJECT.DETAIL` 캐시의 `root` 로
