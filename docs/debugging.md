...
### 4.4 퀵오픈 실패 사후 판별 (로그만으로)

> d-58 §1.G G8 에서 추가한 상주 로그. 계측 코드가 아니라 상시 기록이라 원복 대상이 아니다.
> "퀵오픈에서 파일을 골랐는데 아무 일도 안 일어난다"는 제보는 원인이 셋인데 화면에는 토스트
> 하나로만 나타난다 — 아래 3줄이 그 셋을 로그(§1)만으로 갈라 준다.

| 로그 라인 (레벨) | 뜻 | 다음 수 |
|------------------|-----|---------|
| `파일 탭 열기 선검증 실패 (path=…, 사유=파일 부재)` (warn) | 인덱스는 그 경로를 알지만 디스크에 없다 — 인덱스가 낡았다(이름 변경·삭제 후 재-walk 전) | 정상 동작. 재현되면 워처가 그 변경을 놓쳤는지 본다(`fs:rescan-required` 절) |
| `파일 탭 열기 선검증 실패 (path=…, 사유=프로젝트 경계 밖)` (warn) | 열려 있는 어느 프로젝트 루트에도 속하지 않는다 | 의도된 거부. CLI 로 연 임시 파일이라면 `cli_opened_paths` 등록 여부를 본다 |
| `focused_pane 이 트리에 없어 첫 leaf 로 폴백했습니다 (이전=…, 현재=…)` (info) | 포커스된 pane 이 사라진 상태였고 서버가 스스로 고쳤다 — **탭은 정상적으로 열렸다** | 반복되면 pane 을 없애는 경로(`normalize`)가 `focused_pane` 을 갱신하지 않는 자리를 찾는다 |
| `search_list_files 완료 (projectId=…, 건수=…, 소요=…ms)` (debug) | 인덱스 walk 자체의 규모·비용 | 건수가 기대보다 적으면 `IGNORED_DIR_NAMES`·`.gitignore`·심링크 필터를, 소요가 크면 프로젝트 크기를 본다 |

`debug` 레벨도 **빌드와 무관하게** 남는다 — `lib.rs` 의 `tauri_plugin_log::Builder` 는 전역 레벨을
건드리지 않아 기본값 `Trace` 그대로고(`tungstenite` 두 크레이트만 `Warn` 으로 낮춘다), `log` 크레이트의
`release_max_level_*` 피처도 쓰지 않는다. 따라서 위 4줄은 설치본 로그에서도 그대로 읽을 수 있다(§1 의 40KB
회전 안에서). 경로 외의 값은 싣지 않는다(시크릿 없음).
...
- 마스킹의 두 경계(검토 F1·F2·F3): ① 발급자 접두는 **단어 시작에서만** 인정한다 —
  `task-1738273645123456`·`disk-cache-…` 안의 `sk-` 는 지워지지 않는다. ② `key=value` 규칙은
  **이름 기반**이라 `?page_token=…` 같은 URL 쿼리의 비밀 아닌 값도 `[redacted:key_value]` 가 될 수
  있다(주소·키 이름은 남으므로 어떤 파라미터였는지는 보인다). ③ 그 규칙은 `=` 바로 앞 단어로만
  판정하므로 `AWS_SECRET_ACCESS_KEY=`·`SSH_PRIVATE_KEY=` 는 걸리지만 `STRIPE_API_KEY_LIVE=` 처럼
  자격증명 단어 뒤에 다른 말이 더 붙는 이름은 걸리지 않는다.
