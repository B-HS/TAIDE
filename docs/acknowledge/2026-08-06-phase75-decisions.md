...
## 4. 범위 축소 확정 (2026-08-06, Phase 7.5 구현 완료 시점)

| 항목 | 결정 |
|------|------|
| **pptx LibreOffice 폴백** | **제외.** 개요 미리보기(슬라이드별 텍스트 + "레이아웃이 원본과 다를 수 있습니다" 안내)로 종결한다. 외부 바이너리 감지·변환 커맨드를 만들지 않는다 |
| **remote-control (5번)** | **보류.** 잠자기 중 동작이 OS 정책상 불가하다는 검토 결과(`docs/research/remote-control.md`)에 따라 범위 재합의 전까지 착수하지 않는다. `features/remote-control.md` §2 의 "IPC 계약에 전송 수단 가정을 넣지 않는다"만 계속 지킨다 |
