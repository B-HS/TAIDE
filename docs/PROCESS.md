...
- [ ] 7.9-1. 한글 씹힘 잔존 — **메인(Fable) 직접**: getTargetRanges 수정으로 미해결.
      추측 수정 대신 IME 이벤트 계측(링버퍼+팔레트 복사 커맨드)을 넣어 실기 이벤트 스트림 확보 후 재분석
- [ ] 7.9-2. AI 배지 반영 지연 — 지연 원인(폴링 주기·히스테리시스·이벤트 경로) 분석 후 단축
- [ ] 7.9-3. 번들 테마 확충 — VSCode 내장 전체·IntelliJ 계열 포트·인기 커뮤니티 테마.
      라이선스 확인(MIT 계열만) 선행. Cursor 자체 테마는 라이선스 불명 시 제외
