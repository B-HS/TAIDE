...
- [ ] M3. infra 역참조 4건 제거 후 파일시스템·watcher·persist·Git/LSP/PTY 자원 구현을 Tauri 없는 infra crate로 이전. 각 adapter 테스트·root/symlink/atomic write·자원 종료 검사 유지.
  ...
  - [ ] M3-O. 검증된 self-write slice를 선별 commit·push합니다.
...
> 현재 세부 실행 항목은 M3입니다. M2는 완료됐고, M3의 infra crate 첫 모듈과 self-write tracker 검증을 마쳤습니다. 파일시스템·watcher·persist·Git/LSP/PTY 자원 이전과 M4~M8은 아직 미완료입니다.
