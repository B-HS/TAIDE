...
> M3까지 완료하고 M4의 font·notification·snippet·system·task·tree·locale·theme·settings·sync·search·plugin·VSIX·file·AI·app 서비스 slice를 분리·검증했습니다. 다음 세부 실행 항목은 M4의 나머지 기능별 서비스 crate 이전입니다. Git 서비스 구현과 plugin overlay 소비는 M4, `asset_protocol`·`navigation_guard` platform adapter는 M6 소유이며 M4~M8과 GUI 실기는 미완료입니다.
