...
- **FR-A4** 앱 사이드바의 프로젝트 항목 아이콘은 그 프로젝트에서 현재 focus 된 content 타입
  ...
  > **보류 (2026-09-04, 사용성 배치 4).** 구현 0건이다 — `FocusKind` 와
  > `project:focus-kind-changed` 는 X-A 배치(2026-08-19)에서 제거됐고, 아이콘은 폴더 고정이었다.
  > 그 자리에 **사용자 지정 아이콘·라벨·색**(`features/layout-shell.md` §2.2)이 들어갔다. 되살릴
  > 경우 사용자 지정이 항상 이기고, 지정이 없는 프로젝트에만 focus 아이콘을 적용한다. 에이전트
  > 상태(FR-H1)는 오버레이 배지로 이미 구현돼 있어 이 보류의 영향을 받지 않는다.
- **FR-A5** 앱 재시작 시 열린 프로젝트 목록·활성 프로젝트가 복원된다.

### B. 탭 / 스플릿 (`features/tabs.md`)

- **FR-B1** content 상단에 메뉴 탭 바를 표시한다.
- **FR-B2** 새 프로젝트는 기본으로 1 Welcome(`TabKind::Welcome`) + 1 터미널 탭을 가진다
  (2026-09-04 정정 — 초안의 "1 파일뷰" 는 구현과 다르다. `features/tabs.md` §2).
