...
## Phase 7.6 — IDE 핵심 루프 보강 (2026-08-06 착수)

> Phase 7.5 완료 후 코드 실측으로 확인한 공백. 3순위 후보는 `docs/backlog.md`.
> **멀티 윈도우는 이번 범위 밖**(백로그) — 규모가 크고 다른 기능의 전제가 아니다.

### 7.6-A 1순위 (없으면 IDE 로 못 쓰는 것)

- [ ] 파일 내 찾기/바꾸기 — Monaco 내장 find 위젯에 키바인딩 배선.
      현재 `⌘F` 가 전역 검색으로만 가서 에디터 포커스 시 분기가 필요하다
- [ ] 전역 찾기 → 바꾸기 — Rust `search_replace` 신설(`SearchReplaceResult`). 미리보기 + 일괄 적용
- [ ] Problems 패널 — LSP 진단이 에디터 안에만 있어 프로젝트 전체 오류를 못 본다
- [ ] git 브랜치 UI — **커맨드 자체가 없다**(`GitBranch` 타입 신설). 목록·전환·생성·삭제
- [ ] 저장 시 포맷 / 자동 저장 — LSP formatting 어댑터는 있는데 트리거가 없다

### 7.6-B 2순위 (체감 큰 편의)

- [ ] 아웃라인/심볼 패널 — `documentSymbol` 어댑터가 이미 있어 패널만 붙이면 된다
- [ ] 키맵 오버라이드 UI — 키맵 구조는 있는데 설정에서 못 바꾼다 (Phase 7 범위였는데 누락)
- [ ] 마크다운 미리보기 — `.md` 분할 미리보기
- [ ] git stash / hunk 단위 되돌리기 — 현재 `gitDiscard` 는 파일 단위뿐 (`GitStashEntry` 타입 신설)
- [ ] 터미널 분할·다중 세션 — 지금은 탭 하나당 세션 하나
- [ ] 최근 프로젝트 목록 — 웰컴 화면이 `recentProjects={[]}` 로 **빈 배열 하드코딩**
- [ ] 드래그&드롭으로 파일/폴더 열기
