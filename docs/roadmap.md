...
- [x] 파일 내 찾기/바꾸기 — `⌘F` 를 에디터 포커스 시 Monaco find 위젯으로 분기(별도 `find` 액션 신설).
      `⇧⌘F` 전역 검색은 문서화된 값이라 유지
- [x] 전역 찾기 → 바꾸기 — Rust `search_replace`(원자적 쓰기, 바이너리·비UTF8 skip) + 확인 다이얼로그
- [x] Problems 패널 — Monaco 전역 마커를 읽어 파일별 그룹핑
- [x] git 브랜치 UI — Rust 커맨드 9종 신설 + `BranchSwitcher`(목록·전환·생성)
- [x] 저장 시 포맷 / 자동 저장
...
- [x] 키맵 오버라이드 UI
- [x] 마크다운 미리보기
- [x] git stash / hunk 단위 되돌리기 — **Rust 커맨드까지만.** UI 미착수(아래)
- [x] 터미널 분할·다중 세션
- [x] 최근 프로젝트 목록
- [x] 드래그&드롭으로 파일/폴더 열기
