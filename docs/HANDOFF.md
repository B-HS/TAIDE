...
> **릴리스**: 숫자 `4` 금지 규칙에 따라 v0.2.4를 건너뛴 `v0.2.5`, 태그 `10431dd`. Release 런 `35695586141` 완주(wall 약 9m05s)로 draft가 생성됐고 GitHub 관찰값상 2026-09-22 06:45:50Z에 공개됐습니다. `TAIDE_0.2.5_aarch64.dmg` 15,321,084B와 `SHA256SUMS.txt`를 내려받아 SHA-256 `dc0823f95e8e2fff52dda912aa2335e9b031854ea2a8401f09391742943e08d8` 일치를 확인했습니다.
> **사용자 잔여**: 설치본 실기 확인. 실제 macOS 알림 권한 상태 조회는 현재 플러그인 계약상 불가능하므로 후속 후보입니다.
...
> **반영 커밋(dev=main)**: `7b5dbaf`(editor: 폴백 구문 전용·세션 중 정지/복원·세션 실패 warn) `b8a9aef`(lsp: 감지·spawn·종료 로그·stderr tail 마스킹·PATH 보정 성공 로그) `b9ac6f3`(tree: 하드 제외 해제) `fc1d927`(docs) `f256ad7`(release). 구현 wf `wf_f0dd012e`(fixer-rust ∥ fixer-ts opus·xhigh → verify sonnet·high → 렌즈 1 sonnet·xhigh: major 0·minor 1 메인 정정·info 1 QA 부채). fixer-ts 가 계약 전제 2건 정정: 0.56 의 defaults 는 `monaco.typescript`(`languages.typescript` 는 타입 전용 스텁), `setModeConfiguration` 은 등록된 provider 를 소급 해제 못 해 정지 시 `setDiagnosticsOptions` 동반(세션 중 완성·호버 중복은 알려진 한계, editor.md §12). 검증: `bun run verify` exit 0(bun 2745/0·cargo 1661+4+3+8+17/0·clippy 0)·vite build·typecheck:e2e.
> **릴리스**: `v0.2.2` 태그 `f256ad7`, Release 런 `35071071392` 완주(wall 약 9m12s, dmg 15,280,748B — `deployment.md` §9). draft 유지 — 공개는 사용자.
> **사용자 잔여(실기, v0.2.2 설치본)**: ① 설정 → LSP 서버의 vtsls 행(설치됨/미설치·경로) ② 상태바 `LSP n/m` ③ `TAIDE.log` 의 `lsp detect: vtsls available=…`/`lsp vtsls: spawn …`/`exited …` 줄 — `available=false` 면 계약 §4 PATH 보강 착수 ④ gumba `.ts` 밑줄 소멸(세션 없어도 구문 오류만) ⑤ 트리에 `node_modules/`·`.next/`·`.git/` 표시·큰 `node_modules/` 펼침 체감 ⑥ 세션 중 완성 목록 중복 여부(알려진 한계 — 내장 TS 기여 제거 여부는 제품 판단). 트리 전부 표시에 `.git/` 도 포함됨 — 숨김 원하면 재지시.

> 직전 스냅샷(2026-09-16 2차, d-63·v0.2.1) 헤더는 아래 유지.
...
1. draft **v0.1.3·v0.1.5·v0.1.6·v0.1.7·v0.1.8·v0.1.9 Publish** (누적 — 본문·산출물 검토 후). **v0.1.9**(런 `34052313011`, dmg 15,082,500B)가
   이 세션의 전 배치를 담은 설치본이며 배지 실기 확인 대상이다.
