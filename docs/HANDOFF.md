...
> 최종 갱신: 2026-08-28(후반) / 이 세션의 커밋 완료분: 3차분(d-45·d-46) + 아이콘 + 태그4 금지 +
> docs 전면 정합(`020d523`) + 릴리스 수리분(build·ci·docs — 이 문서 포함) — 전부 main 병합·푸시.
> **릴리스 1차 런은 서명 시크릿 base64 로 실패** → 로컬 선검증 스크립트 2종으로 시크릿 신규
> 등록·[통과] 확인 후 **`v0.1.0` 태그 재발행으로 재시도**(사용자 지시). CI 경고 2건(collision·
> dead_code)도 근본 해소. 커밋 규칙: 대화 내 명시 지시 우선(feedback 2026-08-27 정본, stash 금지).
...
- **즉시**: ① `v0.1.0` **릴리스 완주(4차 런)** — draft Release 검토·공개 + dmg 실기 설치
  확인(사용자) ② qa6 실기 계속(발견 결함은 d-47… 계약 파이프라인). 실패 3건의 원인·수정은
  `deployment.md` §7 트러블슈팅 정본(시크릿 base64·번들 경로·Homebrew dylib 정적화).
...
| e2e 상태 | 완주 판정(§3.1). 픽스처=`~/Library/Caches/net.gumyo.taide.dev/e2e-fixtures`(d-49 분리+2026-08-29 identifier 개명 반영 — Vite 루트 밖+FSEvents 가능, `e2e/lib/paths.ts` JSDoc 정본). 포트는 하네스가 dev 로그 `원격 접속 서버 기동: port=` 에서 발견. 앱 재시작 직후 첫 런은 05 가 vtsls 콜드로 실패 가능(기지). 스위트 2번째부터 C8 가능(자가 회복·dev 전용) |
| 사용자 실기 환경 | 활성 테마 monokai-dimmed 관측(darcula 에서 변경됨)·formatOnSave true·fixAll/organizeImports false·REMOTE 활성+password_only ON·로그(d-49 분리+개명 후): dev=`~/Library/Logs/net.gumyo.taide.dev/TAIDE.log`·설치본=`~/Library/Logs/net.gumyo.taide/TAIDE.log`(UTC·40KB 회전) |
