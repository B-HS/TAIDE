...
## d-62 2a 창 크롬 승격 재검 (2026-09-15, 계약: docs/acknowledge/2026-09-15-d62-project-split-groups-contract.md)

> `window_chrome`(zen·사이드바 레일) 1회 승격은 **한 프로젝트의 값 1벌만** 세션으로 올리고 열려 있던
> 나머지 프로젝트의 레거시 값은 초기화한다(렌즈 검토 S-102, `docs/data-model.md` §22). 값 손실 자체는
> 수용한 트레이드오프이므로, 실기에서 확인할 것은 "손실이 없는가"가 아니라 **이행이 1회로 끝나는가**다.

- [ ] (창 크롬 승격 1회성) 업데이트 전 세션에서 서로 다른 프로젝트 2개 이상에 Zen/사이드바 레일 접힘을
      제각각 남긴 상태로 부팅 → 창 크롬이 활성 프로젝트의 값 한 벌로 정해지고(나머지 프로젝트 값은
      초기화), 이어서 Zen 을 껐다 켜고 앱을 재시작해도 **낡은 Zen 이 되살아나지 않는가**
