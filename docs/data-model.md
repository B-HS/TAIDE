...
> `net.gumyo.taide.dev`, 설치본은 `net.gumyo.taide`(구 `dev.taide.app.dev`/`dev.taide.app` —
> 2026-08-29 개명). `app_data_dir` 를 포함한 이 절 전체(위 트리)도 마찬가지로 identifier 별로
> 갈라진다.
...
- **`TabKind::AppFile`** — 프로젝트 폴더 밖에 있는 앱 소유 파일(`settings.json`, 프롬프트 템플릿
  ...
  - **hot-exit 미러는 1차 제외**(계약 명시) — 저장하지 않은 `AppFile` 탭 내용은 창이 비정상 종료되면
    복구되지 않는다.
  - **"닫기 시 확인" 은 `AppFile` 에 적용되지 않는다(2026-09-17 정정)**. 이 줄과 Wave I 계약이 전제한 "기존
    파일 탭의 dirty 닫기 흐름을 그대로 재사용한다" 는 당시 **그 흐름 자체가 없었다**는 점에서 사실이 아니었다 —
    `tabs.md` §8 이 요구하던 저장/버리기/취소 다이얼로그는 d-67 에 와서야 구현됐고(`tabs.md` §8.1), 그 게이트
    대상은 `TabKind::File` 과 `Untitled` 의 dirty 탭**뿐**이다. `AppFile` 탭은 종전대로 확인 없이 닫힌다. dirty
    표시(탭 바 점) 자체는 다른 탭과 같은 `layout_set_dirty` 경로를 쓴다. 미러가 없어 닫기가 지울 초안도 없다는
    점이 `File` 탭과 다르고, 확장한다면 미러 도입과 함께 판단할 문제다.
